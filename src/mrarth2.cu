...
__FUNCTION_HEADER__ mr_small normalise(_MIPD_ big x, big y) { /* normalise divisor */
    mr_small norm, r;
#ifdef MR_FP
    mr_small dres;
#endif
...
__FUNCTION_HEADER__ void multiply(_MIPD_ big x, big y, big z) {  /*  multiply two big numbers: z=x.y  */
...
__FUNCTION_HEADER__ void divide(_MIPD_ big x, big y, big z) {  /*  divide two big numbers  z=x/y : x=x mod y  *
...
__FUNCTION_HEADER__ BOOL divisible(_MIPD_ big x, big y) { /* returns y|x, that is TRUE if y divides x exactly */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void mad(_MIPD_ big x, big y, big z, big w, big q, big r) { /* Multiply, Add and Divide; q=(x*y+z)/w remainder r   *
