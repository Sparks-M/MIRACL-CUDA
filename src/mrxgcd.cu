...
__FUNCTION_HEADER__  mr_small qdiv(mr_small u, mr_small v) { /* fast division - small quotient expected */
    mr_small x = u;
    x -= v;
    if (x < v) return 1;
    x -= v;
...
__FUNCTION_HEADER__ int xgcd(_MIPD_ big x, big y, big xd, big yd, big z) { /* greatest common divisor by Euclids method  *
...
__FUNCTION_HEADER__ int invmodp(_MIPD_ big x, big y, big z) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    int gcd;
...
__FUNCTION_HEADER__ BOOL double_inverse(_MIPD_ big n, big x, big y, big w, big z) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ BOOL multi_inverse(_MIPD_ int m, big *x, big n, big *w) { /* find w[i]=1/x[i] mod n, for i=0 to m-1 *
