...
__FUNCTION_HEADER__ int logb2(_MIPD_ big x) { /* returns number of bits in x */
    int xl, lg2;
    mr_small top;
#ifdef MR_OS_THREADS
...
__FUNCTION_HEADER__ void sftbit(_MIPD_ big x, int n, big z) { /* shift x by n bits */
    int m;
    mr_small sm;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
...
__FUNCTION_HEADER__ void expb2(_MIPD_ int n, big x) { /* sets x=2^n */
    int r, p;
#ifndef MR_ALWAYS_BINARY
    int i;
#endif
...
__FUNCTION_HEADER__ void bigbits(_MIPD_ int n, big x) { /* sets x as random < 2^n */
    mr_small r;
    mr_lentype wlen;
#ifdef MR_FP
    mr_small dres;
