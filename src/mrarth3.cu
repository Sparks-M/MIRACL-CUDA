...
__FUNCTION_HEADER__ void expint(_MIPD_ int b, int n, big x) { /* sets x=b^n */
    unsigned int bit, un;
#ifdef MR_OS_THREADS
...
__FUNCTION_HEADER__ void power(_MIPD_ big x, long n, big z, big w) { /* raise big number to int power  w=x^n *
...
__FUNCTION_HEADER__ BOOL nroot(_MIPD_ big x, int n, big w) {  /*  extract  lower approximation to nth root   *
