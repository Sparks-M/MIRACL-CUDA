...
__FUNCTION_HEADER__ miracl *mr_first_alloc() {
    return (miracl *)cuda_calloc(1, sizeof(miracl));
}

__FUNCTION_HEADER__ void *mr_alloc(_MIPD_ int num, int size) {
    char *p;
#ifdef MR_OS_THREADS
...
__FUNCTION_HEADER__ void mr_free(void *addr) {
    if (addr == NULL) return;
    free(addr);
    return;
}
