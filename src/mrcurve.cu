...
__FUNCTION_HEADER__ void epoint_getrhs(_MIPD_ big x, big y) { /* x and y must be different */
...
__FUNCTION_HEADER__ BOOL epoint_set(_MIPD_ big x, big y, int cb, epoint *p) { /* initialise a point on active ecurve            *
...
__FUNCTION_HEADER__ void epoint_getxyz(_MIPD_ epoint *p, big x, big y, big z) { /* get (x,y,z) coordinates */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ int epoint_get(_MIPD_ epoint *p, big x, big y) { /* Get point co-ordinates in affine, normal form       *
...
__FUNCTION_HEADER__ BOOL epoint_norm(_MIPD_ epoint *p) { /* normalise a point */
...
__FUNCTION_HEADER__ BOOL epoint_multi_norm(_MIPD_ int m, big *work,
...
__FUNCTION_HEADER__ void ecurve_double_add(_MIPD_ epoint *a, epoint *b, epoint *c, epoint *d, big *s1, big *s2) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void ecurve_multi_add(_MIPD_ int m, epoint **x, epoint **w) { /* adds m points together simultaneously, w[i]+=x[i] */
    int i, *flag;
    big *A, *B, *C;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
...
__FUNCTION_HEADER__ void ecurve_double(_MIPD_ epoint *p) { /* double epoint on active ecurve */
...
__FUNCTION_HEADER__  BOOL ecurve_padd(_MIPD_ epoint *p, epoint *pa) { /* primitive add two epoints on the active ecurve - pa+=p;   *
...
__FUNCTION_HEADER__ void epoint_copy(epoint *a, epoint *b) {
...
__FUNCTION_HEADER__ BOOL epoint_comp(_MIPD_ epoint *a, epoint *b) {
    BOOL result;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ int ecurve_add(_MIPD_ epoint *p, epoint *pa) {  /* pa=pa+p; */
...
__FUNCTION_HEADER__ void epoint_negate(_MIPD_ epoint *p) { /* negate a point */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    if (mr_mip->ERNUM) return;
...
__FUNCTION_HEADER__ int ecurve_sub(_MIPD_ epoint *p, epoint *pa) {
    int r;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ int ecurve_mult(_MIPD_ big e, epoint *pa, epoint *pt) { /* pt=e*pa; */
...
__FUNCTION_HEADER__ void ecurve_multn(_MIPD_ int n, big *y, epoint **x, epoint *w) { /* pt=e[o]*p[0]+e[1]*p[1]+ .... e[n-1]*p[n-1]   */
    int i, j, k, m, nb, ea;
    epoint **G;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
...
__FUNCTION_HEADER__  BOOL ecurve_add_sub(_MIPD_ epoint *P, epoint *Q, epoint *PP, epoint *PM) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    big t1, t2, lam;
...
__FUNCTION_HEADER__ void ecurve_mult2(_MIPD_ big e, epoint *p, big ea, epoint *pa, epoint *pt) { /* pt=e*p+ea*pa; */
    int e1, h1, e2, h2, bb;
    epoint *p1, *p2, *ps[2];
#ifdef MR_STATIC
