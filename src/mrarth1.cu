...
__FUNCTION_HEADER__ void mr_pmul(_MIPD_ big x, mr_small sn, big z) {
    int m, xl;
    mr_lentype sx;
...
__FUNCTION_HEADER__ void premult(_MIPD_ big x, int n, big z) { /* premultiply a big number by an int z=x.n */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ mr_small mr_sdiv(_MIPD_ big x, mr_small sn, big z)
#endif
{
    int i, xl;
    mr_small sr, *xg, *zg;
...
__FUNCTION_HEADER__ int subdiv(_MIPD_ big x, int n, big z) {  /*  subdivide a big number by an int   z=x/n  *
...
__FUNCTION_HEADER__ int remain(_MIPD_ big x, int n) { /* return integer remainder when x divided by n */
    int r;
    mr_lentype sx;
#ifdef MR_FP
    mr_small dres;
...
__FUNCTION_HEADER__ BOOL subdivisible(_MIPD_ big x, int n) {
    if (remain(_MIPP_ x, n) == 0) return TRUE;
    else return FALSE;
}

__FUNCTION_HEADER__ int hamming(_MIPD_ big x) {
    int h;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void bytes_to_big(_MIPD_ int len, const char *ptr, big x) { /* convert len bytes into a big           *
...
__FUNCTION_HEADER__ int big_to_bytes(_MIPD_ int max, big x, char *ptr, BOOL justify) { /* convert positive big into octet string */
...
__FUNCTION_HEADER__ void mr_jsf(_MIPD_ big k0, big k1, big u0p, big u0m, big u1p, big u1m) {
