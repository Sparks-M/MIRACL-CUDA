...
__FUNCTION_HEADER__ void kill_monty(_MIPDO_) {
#ifdef MR_OS_THREADS
...
__FUNCTION_HEADER__ mr_small prepare_monty(_MIPD_ big n) { /* prepare Montgomery modulus */
#ifdef MR_KCM
    int nl;
#endif
#ifdef MR_PENTIUM
...
__FUNCTION_HEADER__ void nres(_MIPD_ big x, big y) { /* convert x to n-residue format */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void redc(_MIPD_ big x, big y) { /* Montgomery's REDC function p. 520 */
...
__FUNCTION_HEADER__ void nres_complex(_MIPD_ big a, big b, big r, big i) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    if (mr_mip->ERNUM) return;
...
__FUNCTION_HEADER__ void nres_lazy(_MIPD_ big a0, big a1, big b0, big b1, big r, big i) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    if (mr_mip->ERNUM) return;
...
__FUNCTION_HEADER__ void nres_dotprod(_MIPD_ int n, big *x, big *y, big w) {
    int i;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void nres_negate(_MIPD_ big x, big w) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    if (size(x) == 0) {
...
__FUNCTION_HEADER__ void nres_div2(_MIPD_ big x, big w) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void nres_div3(_MIPD_ big x, big w) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void nres_div5(_MIPD_ big x, big w) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void nres_double_modadd(_MIPD_ big x, big y, big w) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
#ifdef MR_COMBA
...
__FUNCTION_HEADER__ void nres_double_modsub(_MIPD_ big x, big y, big w) {
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
#ifdef MR_COMBA
...
__FUNCTION_HEADER__ void nres_modadd(_MIPD_ big x, big y, big w) { /* modular addition */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
#ifdef MR_COUNT_OPS
...
__FUNCTION_HEADER__ void nres_modsub(_MIPD_ big x, big y, big w) { /* modular subtraction */
...
__FUNCTION_HEADER__ int nres_moddiv(_MIPD_ big x, big y, big w) { /* Modular division using n-residues w=x/y mod n */
    int gcd;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
...
__FUNCTION_HEADER__ void nres_premult(_MIPD_ big x, int k, big w) { /* multiply n-residue by small ordinary integer */
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    int sign = 0;
...
__FUNCTION_HEADER__ void nres_modmult(_MIPD_ big x, big y, big w) { /* Modular multiplication using n-residues w=x*y mod n */
...
__FUNCTION_HEADER__ BOOL nres_double_inverse(_MIPD_ big x, big y, big w, big z) { /* find y=1/x mod n and z=1/w mod n */
...
__FUNCTION_HEADER__ BOOL nres_multi_inverse(_MIPD_ int m, big *x, big *w) { /* find w[i]=1/x[i] mod n, for i=0 to m-1 *
...
__FUNCTION_HEADER__ void ecurve_init(_MIPD_ big a, big b, big p, int type) { /* Initialize the active ecurve    *
